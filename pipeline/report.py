...
WEEKLY_SYS = (
    ...
    "1. 全程简体中文。\n"
    "2. 本周综述：总分结构，依次输出三个小块，每块以粗体标签开头："
    "**政策面**：…（80字内）、**产业面**：…（80字内）、**市场面**：…（60字内）。"
    "禁止写成大段落。\n"
...
MONTHLY_SYS = (
    ...
    "1. 全程简体中文。\n"
    "2. 本月综述：总分结构，依次输出四个小块，每块以粗体标签开头："
    "**政策面**：…（100字内）、**产业面**：…（100字内）、**市场面**：…（80字内）、"
    "**整体判断**：…（一句话，50字内）。禁止写成大段落，禁止输出「一是…二是…」式长句堆叠。\n"
    "3. 本月重点：按重要度排序，每条以 **粗体标题** 开头（12~20字），后接一句话概述，末尾附 [查看原文](URL)（MM-DD）日期，同一事件合并。\n"
    "4. 数据与价格趋势。\n"
    "5. 下月展望。\n"
    "6. 只基于给定候选，不得编造；直接输出 Markdown，不要前言后记。"
)
